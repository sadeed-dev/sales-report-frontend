...
export function Header({ title, description }: HeaderProps) {
  return (
    <div className="border-b border-slate-200/50 bg-white/80 backdrop-blur-xl sticky top-0 z-10 shadow-sm">
      <div className="px-4 lg:px-8 py-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="min-w-0">
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900 truncate">{title}</h1>
            <p className="text-slate-500 text-xs md:text-sm mt-1 truncate">{description}</p>
          </div>
