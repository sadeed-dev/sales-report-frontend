...
export default function TataCallLogsReport() {
  ...
  const handleExportImage = async () => {
    ...
    try {
      ...
      a.href = url2
      a.download = `tata-call-logs-${new Date().toISOString().split('T')[0]}.zip`
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const a = document.createElement('a')
      a.href = url2
      a.download = `tata-call-logs-${new Date().toISOString().split('T')[0]}.xlsx`
      document.body.appendChild(a)
      a.click()
      window.URL.revokeObjectURL(url2)
      document.body.removeChild(a)
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-10">
        ...
        <span className="ml-3 text-gray-600">Loading Tata call logs...</span>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="text-red-600 p-6">Error loading Tata call logs: {error.message}</div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-sm border border-slate-200 hover:shadow-lg transition-all duration-300">
      ...
      <div className="sticky top-0 z-20 px-6 py-4 bg-gradient-to-r from-blue-50 to-slate-50 border-b border-slate-200 shadow-sm">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 disabled:bg-green-400 text-white rounded-lg text-sm font-semibold transition-colors"
          >
            {isExportingExcel ? (
              <>
          ...
          <button
            ...
            disabled={isExportingExcel || isExportingImage}
            className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-400 text-white rounded-lg text-sm font-semibold transition-colors"
          >
            {isExportingImage ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin" />
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-200 bg-gradient-to-r from-slate-50 to-blue-50/30">
              ...
              <SortHeader label="Avg Talk Time" sortKey="avg_talk_time_sec" />
          ...
          <tbody>
            {sortedData.map((row, index) => (
              ...
              >
                <td className="px-6 py-4 text-sm font-semibold text-slate-900">{row.sr_no}</td>
                <td className="px-6 py-4 text-sm font-semibold text-slate-900">{row.user_name}</td>
                <td className="px-6 py-4 text-sm text-slate-600">{row.phone_number || '-'}</td>
                <td className="px-6 py-4 text-sm text-slate-600">{row.branch || '-'}</td>
                <td className="px-6 py-4 text-sm font-bold text-slate-900">{row.total_calls}</td>
      ...
      {row.not_connected_calls}
                ...
                <td className="px-6 py-4 text-sm font-mono font-semibold text-slate-900">{formatSecondsToHMS(row.total_talk_time_sec)}</td>
                <td className="px-6 py-4 text-sm font-mono font-semibold text-slate-900">{formatSecondsToHMS(row.avg_talk_time_sec)}</td>
                {/* <td className="px-6 py-4 text-sm font-semibold text-blue-600">{row.talktimePercent}</td> */}
              </tr>
            ))}
          </tbody>
        </table>
